...
makes an existing screenshot stale, update `capture.js` to match (it's a
project asset to maintain, not a scratch script to throw away after one
session) and re-run it** rather than hand-capturing new ones or leaving
the old ones stale. Confirmed working end to end as of 2026-08-14 in this
project's sandbox environment specifically — see that session's task log
entry for the exact setup steps that got a live backend+frontend+PostGIS
running here, since `docker-compose` alone doesn't work in this sandbox
(see the script's README for why and the fallback).
